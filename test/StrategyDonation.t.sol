...
contract StrategyDonationTest is Test {
    ...
    function testDonationIncreasesRedemptionValue() public {
        ...
        assertEq(monstr.balanceOf(alice), expectedTokens, "Alice should receive 9,900 tokens");
        assertEq(monstr.balanceOf(monstr.FEES_POOL()), expectedFee, "Fees pool should have 100 tokens");
        ...
        assertEq(address(monstr).balance, contractBalanceBefore + donationAmount, "Contract balance should increase by 5 ETH");

        // Check total supply unchanged
        assertEq(monstr.totalSupply(), totalSupplyBefore, "Total supply should remain 10,000 tokens");

        // Calculate redemption value after donation
        uint256 redemptionValueAfter = (netAmount * address(monstr).balance) / monstr.totalSupply();
    ...
    function testDonationDoesntAffectMinting() public {
        ...
        assertEq(monstr.balanceOf(alice), 990 ether, "Alice should get 990 tokens");
        ...
        (bool success,) = address(monstr).call{value: donationAmount}("");
        assertTrue(success, "Donation should succeed");
        assertEq(address(monstr).balance, 11 ether, "Contract should have 11 ETH");
        ...
        assertEq(monstr.balanceOf(bob), 990 ether, "Bob should get 990 tokens despite donation");
        assertEq(address(monstr).balance, 12 ether, "Contract should have 12 ETH");
    }
    ...
    function testDonationDoesntBreakLottery() public {
        ...
        vm.prank(bob);
        monstr.mint{value: 10 ether}();
        assertEq(monstr.balanceOf(bob), 9900 ether, "Bob should have 9,900 tokens");
        ...
        assertEq(monstr.balanceOf(alice), 8900 ether, "Alice should have 8,900 tokens");
        assertEq(monstr.balanceOf(bob), 10890 ether, "Bob should have 10,890 tokens");
        assertEq(monstr.balanceOf(monstr.FEES_POOL()), 210 ether, "Fees pool should have 210 tokens");
        ...
        (bool success,) = address(monstr).call{value: donationAmount}("");
        assertTrue(success, "Donation should succeed");
        assertEq(address(monstr).balance, 25 ether, "Contract should have 25 ETH");
        ...
        assertEq(monstr.balanceOf(bob), 9890 ether, "Bob should have 9,890 tokens");
        assertEq(monstr.balanceOf(alice), 9890 ether, "Alice should have 9,890 tokens");
        assertEq(monstr.balanceOf(monstr.FEES_POOL()), 220 ether, "Fees pool should have 220 tokens");
        ...
        assertEq(monstr.lastLotteryDay(), 2, "Lottery should be executed for day 2");
    }
    ...
    function testRedemptionWithDonation() public {
        ...
        (bool success,) = address(monstr).call{value: donationAmount}("");
        assertTrue(success, "Donation should succeed");
        assertEq(address(monstr).balance, 15 ether, "Contract should have 15 ETH");
        ...
        uint256 expectedEth = (netRedeemed * 15 ether) / monstr.totalSupply(); // (4900.5 * 15) / 10000 = 7.35075 ETH
        ...
        assertEq(monstr.balanceOf(alice), 4950 ether, "Alice should have 4,950 tokens left");
    }
